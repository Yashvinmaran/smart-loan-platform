...
export default function AdminNewVerifiedLoansPage() {
  ...
  async function fetchLoans() {
    setLoading(true);
    try {
      const res = await AdminAPI.getLoans({ page: 0, size: 50, status: 'NEW_VERIFIED' });
      ...
      console.error('Failed to fetch new verified loans', e);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="admin-wrap">
      ...
      <main className={`admin-main ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
        <header className="admin-header">
          ...
          <h2>New Verified Loan Applications</h2>
        </header>
